...
func (lWriter *logWriter) println(when time.Time, msg string) {
	lWriter.Lock()
	defer lWriter.Unlock()
	h, _ := formatTimeHeader(when)
	lWriter.writer.Write(append(append(h, msg...), '\n'))
}
...
func NewAnsiColorWriter(writer io.Writer) io.Writer {
	return NewModeAnsiColorWriter(writer, DiscardNonColorEscSeq)
}

func NewModeAnsiColorWriter(writer io.Writer, mode outputMode) io.Writer {
	if _, ok := writer.(*ansiColorWriter); !ok {
...
func formatTimeHeader(when time.Time) ([]byte, int) {
	...
	buf[0] = y1[y/1000%10]				// 년의 첫자리
	buf[1] = y2[y/100]					// 년의 두번째 자리
	buf[2] = y3[y-y/100*100]			// 년의 세번째 자리
	buf[3] = y4[y-y/100*100]			// 년의 네번째 자리
	buf[4] = '/'
	buf[5] = mo1[mo-1]
	buf[6] = mo2[mo-1]
	buf[7] = '/'
